...
echo "[INFO] Building Authorization Service..."
...
echo "[INFO] Running Authorization Service migrations..."
...
# Note: OPENFGA_AUTHORIZATION_MODEL_ID=dummy is required because Authorization Service config validation
...
if ! err_msg=$(POSTGRES_PORT=5433 POSTGRES_USER=authorization-service POSTGRES_PASSWORD=password OPENFGA_STORE_ID=01GP1254CHWJC1MNGVB0WDG1T0 OPENFGA_API_KEY=42 OPENFGA_AUTHORIZATION_MODEL_ID="$MODEL_ID" \
    ...
    echo "  [WARN] Authorization Service rule seeding failed: $err_msg" >&2
else
    echo "  ✓ Authorization Service route rules seeded"
fi
...
curl -s -X POST http://localhost:8082/stores/01GP1254CHWJC1MNGVB0WDG1T0/write \
  ...
  -d '{"writes": {"tuple_keys": [{"user": "user:domain-admin@canonical.com", "relation": "assignee", "object": "role:admin", "condition": {"name": "tenant_match", "context": {"tenant": "hook-service"}}}]}}' >/dev/null
...
# Start Authorization Service extAuthz Serve
start_daemon "Authorization Service Serve" 9091 "$LOGS_DIR/authorization-service-serve.log" \
...
# Start Authorization Service Async Worker
start_daemon "Authorization Service Async Worker" 9102 "$LOGS_DIR/authorization-service-worker.log" \
...
# Start Authorization Service Kafka Listener
start_daemon "Authorization Service Kafka Listener" 9101 "$LOGS_DIR/authorization-service-listener.log" \
