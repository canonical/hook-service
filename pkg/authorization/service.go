...
func (s *Service) RemoveAllowedAppsFromGroup(ctx context.Context, groupID string) error {
	ctx, span := s.tracer.Start(ctx, "authorization.Service.RemoveAllowedAppsFromGroup")
	defer span.End()
	...
	// TODO: use group name instead when group API is implemented
	err = s.authz.RemoveAllowedAppsFromGroup(ctx, groupID)
	if err != nil {
...
func (s *Service) RemoveAllowedGroupsForApp(ctx context.Context, app string) error {
	...
	groups, err := s.db.RemoveAllowedGroupsForApp(ctx, app)
	if err != nil {
		s.logger.Error(err.Error())
		return err
	}
	...
	// TODO: use group name instead when group API is implemented
	if err := s.authz.RemoveAllowedGroupsForApp(ctx, app); err != nil {
