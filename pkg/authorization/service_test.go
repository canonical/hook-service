...
func TestServiceGetAllowedAppsInGroup(t *testing.T) {
...
func TestServiceAddAllowedAppToGroup(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectedError != nil {
				mockLogger.EXPECT().Error(test.expectedError.Error())
			}
			mockTracer := NewMockTracingInterface(ctrl)
			mockMonitor := NewMockMonitorInterface(ctrl)
...
func TestServiceRemoveAllowedAppsFromGroup(t *testing.T) {
	dbErr := errors.New("some db error")
	authzErr := errors.New("some authz error")
	groupID := "group1"
	...
	tests := []struct {
	...
	}{
		{
			...
			mockAuthorizer: func(ctrl *gomock.Controller) AuthorizerInterface {
				mock := NewMockAuthorizerInterface(ctrl)
				mock.EXPECT().RemoveAllowedAppsFromGroup(gomock.Any(), groupID).Return(nil)
				return mock
			},
			expectedError: nil,
		},
		...
		},
		{
			...
			mockAuthorizer: func(ctrl *gomock.Controller) AuthorizerInterface {
				mock := NewMockAuthorizerInterface(ctrl)
				mock.EXPECT().RemoveAllowedAppsFromGroup(gomock.Any(), groupID).Return(authzErr)
				return mock
			},
			expectedError: authzErr,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mockTracer.EXPECT().Start(gomock.Any(), "authorization.Service.RemoveAllowedAppsFromGroup").Times(1).Return(context.TODO(), trace.SpanFromContext(context.TODO()))

			s := NewService(mockDB, mockAuthorizer, mockTracer, mockMonitor, mockLogger)

			err := s.RemoveAllowedAppsFromGroup(context.TODO(), groupID)
...
func TestServiceRemoveAllowedAppFromGroup(t *testing.T) {
	dbErr := errors.New("some db error")
	authzErr := errors.New("some authz error")
	groupID := "group1"
	app := "app1"
...
func TestServiceGetAllowedGroupsForApp(t *testing.T) {
	dbErr := errors.New("some db error")
	app := "app1"
...
func TestServiceRemoveAllowedGroupsForApp(t *testing.T) {
	dbErr := errors.New("some db error")
	authzErr := errors.New("some authz error")
	app := "app1"
	groups := []string{"g1", "g2"}
	...
	tests := []struct {
	...
	}{
		{
			...
			mockDB: func(ctrl *gomock.Controller) AuthorizationDatabaseInterface {
				mock := NewMockAuthorizationDatabaseInterface(ctrl)
				mock.EXPECT().RemoveAllowedGroupsForApp(gomock.Any(), app).Return(groups, nil)
				return mock
			},
			mockAuthorizer: func(ctrl *gomock.Controller) AuthorizerInterface {
				mock := NewMockAuthorizerInterface(ctrl)
				mock.EXPECT().RemoveAllowedGroupsForApp(gomock.Any(), app).Return(nil)
				return mock
			},
			expectedError: nil,
		},
		{
			...
			mockDB: func(ctrl *gomock.Controller) AuthorizationDatabaseInterface {
				mock := NewMockAuthorizationDatabaseInterface(ctrl)
				mock.EXPECT().RemoveAllowedGroupsForApp(gomock.Any(), app).Return(nil, dbErr)
				return mock
			},
			mockAuthorizer: func(ctrl *gomock.Controller) AuthorizerInterface {
				return NewMockAuthorizerInterface(ctrl)
		...
		},
		{
			...
			mockAuthorizer: func(ctrl *gomock.Controller) AuthorizerInterface {
				mock := NewMockAuthorizerInterface(ctrl)
				mock.EXPECT().RemoveAllowedGroupsForApp(gomock.Any(), app).Return(authzErr)
				return mock
			},
			expectedError: authzErr,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mockTracer.EXPECT().Start(gomock.Any(), "authorization.Service.RemoveAllowedGroupsForApp").Times(1).Return(context.TODO(), trace.SpanFromContext(context.TODO()))

			s := NewService(mockDB, mockAuthorizer, mockTracer, mockMonitor, mockLogger)

			err := s.RemoveAllowedGroupsForApp(context.TODO(), app)
