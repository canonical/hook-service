...
func TestSalesforceImporter_ImportUserGroups(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			importer := NewSalesforceImporter(mockStorage, mockTracer, mockMonitor, mockLogger)

			mockTracer.EXPECT().Start(gomock.Any(), "groups.SalesforceImporter.ImportUserGroups").Return(context.Background(), trace.SpanFromContext(context.Background()))
			tc.setupMocks(mockStorage, mockSF, mockLogger)

			count, err := importer.ImportUserGroups(context.Background(), mockSF)
