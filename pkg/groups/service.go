...
func (s *Service) CreateGroup(ctx context.Context, name, organization, description string, gType types.GroupType) (*types.Group, error) {
...
func (s *Service) ListUsersInGroup(ctx context.Context, groupID string) ([]string, error) {
	...
	users, err := s.db.ListUsersInGroup(ctx, groupID)
	if err != nil {
		return nil, err
	}
	return users, nil
...
func (s *Service) RemoveAllUsersFromGroup(ctx context.Context, groupID string) error {
	...
	if _, err := s.db.RemoveAllUsersFromGroup(ctx, groupID); err != nil {
...
func (s *Service) UpdateGroupsForUser(ctx context.Context, userID string, groupIDs []string) error {
	...
	if err := s.db.UpdateGroupsForUser(ctx, userID, groupIDs); err != nil {
		...
		return fmt.Errorf("failed to update groups for user: %v", err)
	}
	return nil
}
