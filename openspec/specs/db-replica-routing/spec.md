...
Offload read-only database queries to a read-only replica pool to reduce load on the primary database during read-heavy operations, such as OAuth token issuance.

This capability exists because routing all queries to the primary database limits scalability and increases latency during traffic spikes. Supporting a read-only replica connection pool improves the service's read latency and resilience, while maintaining data integrity on the primary database.

Key decisions:
- Read-only routing is automatically active for HTTP GET/HEAD requests and explicit gRPC read contexts.
- Fallback to the primary occurs transparently if the replica pool is unavailable, unhealthy, or if the replication lag exceeds the configured threshold.
- Transactions and lazy transaction setups always bypass replica routing to prevent write anomalies.

Non-goals:
- This spec does not define database-level replication setup or failover mechanisms.
- This spec does not define dynamic pool resizing logic during runtime.
