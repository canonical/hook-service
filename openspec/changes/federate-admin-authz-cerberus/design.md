...
**Rationale**: Lost permission messages mean missing OpenFGA tuples, which means authorization checks fail (deny-by-default). At-least-once delivery with idempotent processing on the Cerberus side is the standard pattern. Including the message ID in the idempotency key ensures that transport retries are deduplicated by Cerberus's `permission_update_work` table without permanently blocking re-adding a previously removed user or repeating a batch operation. The DB is the source of truth, so rolling it back for a Kafka delivery failure is worse than a missing tuple.
