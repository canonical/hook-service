...
**Chosen**: `requiredAcks=RequireAll`, `maxAttempts=3`, idempotency keys on each message (`subject:relation:object:op:<message_id>` or `object:batch:<hash>:<message_id>`). Kafka delivery failures do not affect the database transaction — the DB write succeeds regardless.
