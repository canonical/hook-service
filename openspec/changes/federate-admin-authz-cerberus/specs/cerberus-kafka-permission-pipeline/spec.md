...
- At-least-once delivery with idempotency keys (`subject:relation:object:op:<message_id>` or `object:batch:<hash>:<message_id>`) ensures message redeliveries are safely deduplicated by Cerberus while distinct operations (such as re-adding a removed user) generate unique keys.
