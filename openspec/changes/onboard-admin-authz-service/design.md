...
The Authorization Service (`canonical/authorization-service`) provides a centralized authorization platform with a shared OpenFGA store, a modular authorization model, Istio extAuthz for HTTP route-based enforcement, and a Kafka-based async pipeline for permission tuple updates.

This design covers onboarding the admin API (group and membership management) to Authorization Service via a custom `hook-service` module. The token hook endpoint continues using its direct OpenFGA client and its own OpenFGA store.
...
**Goals:**
- Protect admin API endpoints with per-group, role-based authorization via Authorization Service extAuthz
- Publish `group-in-claim` permission updates to Kafka for Authorization Service to apply to its shared OpenFGA store
- Maintain OpenFGA store hygiene by deleting all associated tuples on group deletion
- Disable `AUTHENTICATION_ENABLED` for admin routes (Authorization Service extAuthz handles auth)

**Non-Goals:**
- Migrate the token hook endpoint — it continues using its own OpenFGA client and store
- Migrate hook-service to the Authorization Service shared OpenFGA store — hook-service keeps its own OpenFGA instance
...
**Chosen**: PostgreSQL writes are synchronous within the request transaction. Kafka publishes are fire-and-forget with respect to the transaction — they do not affect the database outcome. If the Kafka publish fails, the database record is still committed and the request succeeds. Tuple application to OpenFGA is async via Authorization Service's worker.

**Rationale**: The `groups` and `group_members` tables are the source of truth for membership listing. Kafka publishes `group-in-claim` tuples so Authorization Service extAuthz can enforce per-group access control on admin API endpoints. Eventual consistency in OpenFGA is acceptable because the Authorization Service worker applies tuples within seconds. The database record is the primary artifact — rolling it back for a Kafka delivery failure is worse than a temporarily missing tuple.

### D4: Kafka producer with at-least-once semantics

**Chosen**: `requiredAcks=RequireAll`, `maxAttempts=2`, idempotency keys on each message (`subject:relation:object:op:<message_id>` or `object:batch:<hash>:<message_id>`). Kafka delivery failures do not affect the database transaction — the DB write succeeds regardless.

**Rationale**: Lost permission messages mean missing OpenFGA tuples, which means authorization checks fail (deny-by-default). At-least-once delivery with idempotent processing on the Authorization Service side is the standard pattern. Including the message ID in the idempotency key ensures that transport retries are deduplicated by Authorization Service's `permission_update_work` table without permanently blocking re-adding a previously removed user or repeating a batch operation. The DB is the source of truth, so rolling it back for a Kafka delivery failure is worse than a missing tuple.

### D5: Disable JWT authentication for admin routes

**Chosen**: Set `AUTHENTICATION_ENABLED=false` to disable JWT middleware on admin routes. Authorization Service extAuthz handles authentication and authorization.
...
**Chosen**: Use `user with tenant_match` on all relations, consistent with every other module in the Authorization Service ecosystem.
...
**Rationale**: PostgreSQL `group_members` is the authoritative source of truth for both member users and the group owner. Proactively publishing individual DELETE events cleans up the OpenFGA store, prevents dangling/stale tuples, and keeps Authorization Service's OpenFGA state synchronized with hook-service without requiring a background reconciliation job. If any Kafka publish fails, a warning is logged while the database transaction succeeds.

### D9: Composite primary key (group_id, role, user_id) for multi-role group membership in PostgreSQL

**Chosen**: Migrate `group_members` from `PRIMARY KEY (group_id, user_id)` to `PRIMARY KEY (group_id, role, user_id)` via a new migration, and scope all member-related mutation and query builders by `role = RoleMember`.

**Rationale**:
1. **Extensibility over dedicated tables**: While a separate `group_owners` table would isolate ownership, it causes table proliferation when adding future roles (`viewer`, `editor`, `admin`). A single `group_members` table with `(group_id, role, user_id)` treats role as a dimension and allows arbitrary future role extensions without schema changes.
2. **Eliminating OpenFGA zombie tuples**: OpenFGA models `owner` and `member` as orthogonal relations. With `(group_id, role, user_id)`, a user can hold both `RoleOwner (1)` and `RoleMember (0)` simultaneously in PostgreSQL. Adding an owner as a member succeeds in both PostgreSQL and OpenFGA. Removing a user from the member endpoint deletes only the `RoleMember` row, preserving the `RoleOwner` row. Subsequent `DeleteGroup` calls discover the owner and cleanly publish `owner: DELETE` to Kafka, preventing permanent zombie tuples.
3. **Query safety & token claim isolation**:
   - Mutation operations (`RemoveUsersFromGroup`, `SyncGroupMembers`, `UpdateGroupsForUser`) are explicitly scoped to `role = RoleMember` to prevent cross-role data deletion.
   - Read operations for token claims (`GetGroupsForUser`, `StreamGroupsForUser`) filter by `role = RoleMember` to prevent duplicate group rows and uphold Decision D2 (administrative ownership alone does not confer token claim membership).
...
| Eventual consistency: tuple not yet applied when admin API is called | Authorization Service worker applies within seconds; admin API is low-traffic |
...
| Two OpenFGA instances (hook-service own store + Authorization Service shared store) | Separate stores are intentional — hook-service's token hook uses its own store for app-to-group checks. Configuration drift is not a concern since the stores serve different purposes. |
...
| Cross-role corruption in single `group_members` table | All member mutation queries (`RemoveUsersFromGroup`, `SyncGroupMembers`, `UpdateGroupsForUser`) are strictly filtered by `role = RoleMember`. |
| Duplicate group claims in token issuance | `GetGroupsForUser` explicitly filters by `role = RoleMember`, guaranteeing single-claim issuance and excluding owner-only users. |