...
- The `group-in-claim` type uses `admin` / `owner` / `member` relations with `user with tenant_match`, consistent with every other module in the Authorization Service ecosystem. `admin` inherits `owner` for domain-level management. `member` represents token claim inclusion and is decoupled from `owner` (an administrative owner is not implicitly a claim member unless explicitly added).
...
Non-goals:
- This spec does not define the Kafka pipeline for publishing permission tuples (see `authorization-service-kafka-permission-pipeline`).
- This spec does not define the Istio extAuthz integration (see `authorization-service-admin-api-extauthz`).
...
### Requirement: hook-service module in Authorization Service
The system SHALL provide a `hook-service` module in the Authorization Service authorization model defining the `group-in-claim` type with `admin`, `owner`, and `member` relations, each accepting `user with tenant_match`.

#### Scenario: Module is loadable
- **WHEN** Authorization Service starts with the `hook-service` module
...
- **THEN** Authorization Service extAuthz returns 403
...
- **THEN** Authorization Service extAuthz allows the request
...
The system SHALL provide a `rules.yaml` file in the Authorization Service repository that maps hook-service admin API HTTP routes to `group-in-claim` permission checks.
...
- **WHEN** Authorization Service extAuthz receives a request to `/api/v0/hook/hydra`, `/api/v0/metrics`, or `/api/v0/status`
...
- **THEN** the Authorization Service extAuthz `owner` check succeeds
