...
Key decisions:
- Use `segmentio/kafka-go` as the Kafka producer library to match Authorization Service's own choice.
- Publish protobuf `PermissionUpdateEnvelope` messages defined in the Authorization Service repository. Hook-service only needs the message types, not the Authorization Service gRPC service definitions.
- At-least-once delivery with idempotency keys (`subject:relation:object:op:<message_id>` or `object:batch:<hash>:<message_id>`) ensures message redeliveries are safely deduplicated by Authorization Service while distinct operations (such as re-adding a removed user) generate unique keys.
...
- User-group membership is published to Kafka (for Authorization Service extAuthz enforcement) AND remains in PostgreSQL `group_members` table (for the token hook's contextual tuples and group lifecycle management). Both systems need this data for their respective purposes.
...
- This spec does not define the Authorization Service-side listener/worker pipeline — only the hook-service publisher side.
- This spec does not define the Kafka infrastructure deployment — topic creation, broker configuration, and the `KAFKA_FEDERATED_SERVICES` env var in Authorization Service are assumed to be handled by operators.
...
- **THEN** the message includes a unique `message_id`, an `idempotency_key` derived from `subject:relation:object:op:<message_id>`, and the current timestamp
...
#### Scenario: Group owner added as member
- **WHEN** `AddUsersToGroup` is called for a user who is already an owner of the group
- **THEN** a new row with `role = RoleMember` is inserted into PostgreSQL alongside the existing `RoleOwner` row
- **THEN** the system publishes `PERMISSION_OP_WRITE` for `user:<user_id> → member → group-in-claim:<group_id>`
...
#### Scenario: User holding both owner and member roles removed from members
- **WHEN** `RemoveUserFromGroup` is called for a user who holds both owner and member roles
- **THEN** only the `role = RoleMember` row is deleted from PostgreSQL; the `RoleOwner` row is preserved
- **THEN** the system publishes `PERMISSION_OP_DELETE` for `user:<user_id> → member → group-in-claim:<group_id>`
- **THEN** the user's `owner` tuple in OpenFGA remains active
...
#### Scenario: Group deletion for user holding both owner and member roles
- **WHEN** `DeleteGroup` is called for a group where a user holds both owner and member roles
- **THEN** both `PERMISSION_OP_DELETE` for `owner` and `PERMISSION_OP_DELETE` for `member` are published for that user
