...
// StorageInterface defines the storage operations needed for importing
...
func (si *SalesforceImporter) ImportUserGroups(ctx context.Context, sfClient salesforce.SalesforceInterface) (int, error) {
	ctx, span := si.tracer.Start(ctx, "importer.SalesforceImporter.ImportUserGroups")
	defer span.End()

	// Query all team members from Salesforce
	var members []salesforce.TeamMember
