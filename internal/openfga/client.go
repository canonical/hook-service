...
func (c *Client) WriteTuples(ctx context.Context, tuples ...Tuple) error {
	...
	ts := make([]openfga.TupleKey, len(tuples))

	for i, tuple := range tuples {
		ts[i] = *openfga.NewTupleKey(tuple.Values())
	}
...
func (c *Client) BatchCheck(ctx context.Context, tuples ...Tuple) (bool, error) {
	...
	for _, check := range *data.Result {
		allowed = allowed && *check.Allowed
		if check.Error != nil {
			errString = append(errString, fmt.Sprintf("* %v", check.Error))
		}
	}
