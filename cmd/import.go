...
Example:
  hook-service import --driver salesforce --dsn "postgres://user:pass@host:5432/db" --domain "example.com" --consumer-key "key" --consumer-secret "secret"`,
...
func init() {
	...
	_ = importCmd.MarkFlagRequired("consumer-key")
	_ = importCmd.MarkFlagRequired("consumer-secret")
...
func runImport(cmd *cobra.Command) error {
	...
	switch driver {
	case "salesforce":
		importDriver, err = buildSalesforceDriver(cmd)
		if err != nil {
			return err
		}
	default:
...
func buildSalesforceDriver(cmd *cobra.Command) (*importer.SalesforceDriver, error) {
	...
	if domain == "" || consumerKey == "" || consumerSecret == "" {
		return nil, fmt.Errorf("salesforce driver requires --domain, --consumer-key, and --consumer-secret flags")
	}
